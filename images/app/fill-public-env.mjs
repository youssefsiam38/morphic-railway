// Write the deployment's public values into Morphic's built files.
//
...
const MARKER = "morphic-railway-placeholder-";
...
const SHAPES = {
  ...
  NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/,
};

const die = (msg) => {
  process.stderr.write(`[morphic-app] FATAL: ${msg}\n`);
  process.exit(1);
};
...
function buildIndex(nextDir, pristineDir) {
  // Morphic runs `next start` from the whole build directory, as upstream's image does. The build
  // cache is not served and is left out of the runtime image.
  const roots = [{ from: nextDir, to: ".next" }];
  const listed = [];
  for (const { from, to } of roots) {
    for (const file of walk(from)) {
      if (file.includes(`${join(from, "cache")}/`)) continue;
...
function fill(appDir, pristineDir) {
  ...
  process.stdout.write(`[morphic-app] public values written into ${files.length} built files\n`);
}

if (process.argv[2] === "--index") buildIndex(process.argv[3], process.argv[4]);
else fill(process.env.MORPHIC_APP_DIR ?? "/app", process.env.MORPHIC_PRISTINE_DIR ?? "/opt/morphic-railway/pristine");
