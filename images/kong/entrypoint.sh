#!/bin/bash
# morphic-railway Kong entrypoint: mint the API keys, render the routes, start Kong.
set -uo pipefail
log()  { printf '[morphic-kong] %s\n' "$*"; }
fail() { printf '[morphic-kong] FATAL: %s\n' "$*" >&2; exit 1; }
...
keys=$(morphic-mint-keys) || fail "could not mint the Supabase API keys"
